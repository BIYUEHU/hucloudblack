 ...
 */
function loadView($file, $data = [], $path = HCB_DEMO_VIEW_PATH)
{
 ...
 * @param string $path 根目录
 */
function loadConfig($file, $path = HCB_CONFIG_PATH)
{
    $path = $path . '/' . $file;
    if (file_exists($path)) {
        $result = require($path);
 ...
 * @param string $path 根目录
 */
function setConfig($data, $file, $path = HCB_CONFIG_PATH)
{
    $path = $path . '/' . $file;
